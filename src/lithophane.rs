...
impl<F: Fn(f32, f32, f32, f32) -> f32> LithophaneGenerator<F> {
    ...
    fn generate_point_cloud(&self, vertices: Vec<Vec3>) -> Result<PointCloud, InvalidPointsError> {
        ...
        for y_i in 0..image_height {
            for x_i in 0..image_width {
                ...
                let norm1 = normalize_to_unit_vector(
                    cross_product(vertices[(y_i + 2) * vw + 1 + x_i] - v, vertices[(y_i + 1) * vw + 2 + x_i] - v)
                )?;
                // upper and left vectors
                let norm2 = normalize_to_unit_vector(
                    cross_product(vertices[y_i * vw + 1 + x_i] - v, vertices[(y_i + 1) * vw + x_i] - v)
                )?;
        ...
        Ok(PointCloud {
            vertices: vertices.into_iter().enumerate().filter(|&(i, _)| {
                let image_width = self.image.width() as usize;
                let image_height = self.image.height() as usize;

                return i >= image_width + 2 // exclude extra bottom row
    ...
    fn generate_mesh(&self, point_cloud: PointCloud) -> Result<Vec<Triangle>, InvalidPointsError> {
        ...
        let get_px_depth = |gray_value: u8| -> f32 {
            self.white_depth + (255 - gray_value) as f32 / 255.0 * (self.black_depth - self.white_depth)
        };
